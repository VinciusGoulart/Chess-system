...
public class ChessMatch {
    ...
    public ChessPiece replacePromotedPiece(String type) {
        ...
        if (!type.equals("N") && !type.equals("B") && !type.equals("Q") && !type.equals("R")) {
            return promoted;
        }
    ...
    private Piece undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        // Undoing Special-move King side castling
        if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
            Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
            Position targetT = new Position(source.getRow(), source.getColumn() + 1);
        ...
        // Undoing Special-move Queen side castling
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
            Position sourceT = new Position(source.getRow(), source.getColumn() - 4);
            Position targetT = new Position(source.getRow(), source.getColumn() - 1);
            ChessPiece rook = (ChessPiece) board.removePiece(targetT);
    ...
    private ChessPiece king(Color color) {
        List<Piece> king = piecesOnBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color).collect(Collectors.toList());
        for (Piece p : king) {
